...
/*----------------------------------------------------------------------------------------------------------------------------------------
	...
	this controller file handles resetting of the admin password
------------------------------------------------------------------------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------------------------------------------------------------------
	Admin Module: 
        This controller file handles resetting of the admin password
...
------------------------------------------------------------------------------------------------------------------------------------------*/ 
class AdminPasswordRemindersController extends Controller {
	...
	/**
	 * Admin password reminder view 
	 * 
	 * @method GET 
	 * @return HTML -> password reminder
	 */
	...
	{	
		...
		$response = Password::admin()->remind(Input::only('email'), function($message, $user, $type, $token){
			$message->subject('AKDN MER Consultant Database: Reset your password');
		});
		...
		if($validator->passes())
		{
			...
			switch ($response)
			{
				...
				case Password::PASSWORD_RESET:
					return Redirect::route('admin.login')->with('message', 'Password updated successfully')->with('message_type','success');
			}
		}
		else
		{
